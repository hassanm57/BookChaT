...
def hit_at_k(chunks: list[dict], expected: int, k: int) -> bool: # this function and the next two are new, added more evaluation metrics to evaluator.py
...
def judge_answer(client, question: str, chunks: list[dict], answer: str) -> dict: # what this does is it takes the question, the retrieved chunks, and the answer generated by the RAG system, and then it uses an LLM (like GPT-4o-mini) to evaluate the answer's quality in terms of faithfulness and relevance. It constructs a prompt with the question, context, and answer, sends it to the LLM, and expects a JSON response indicating whether the answer is faithful and relevant. If there's an error during this process, it returns None for both metrics along with the error message.
